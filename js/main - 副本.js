...
$(document).ready(function(){
    ...
    $('.di_right ul li').find('.di_inp').click(function(){
        ...
        if(inptap =='true'){
            console.log(1);
            $(this).css({'background':'url("http://s-event.cn/EDM/dell/161011reg/images/check1.jpg") no-repeat'}).attr('ifr','false');
            // console.log( $(this).next().text());
            texti++;
            // console.log(texti);
            textArr.push($(this).next().text());
        }else{
            $(this).css({'background':'url("http://s-event.cn/EDM/dell/161011reg/images/check.jpg") no-repeat'}).attr('ifr','true');
            texti--;
    ...
    $('.ag_title ul li').bind('click',function(){
         agtNum = $(this).index();
            $('.ag_title ul li').css({'color': '#ffffff'});
            $(this).css({'color': '#43d1f8'});
            $('.ag_cont').hide();
            $('.ag_cont').eq(agtNum).fadeIn(300);
            if(agtNum == 1){
                $('.ag_af_nav').show();
                $('.line_hover').animate({'left':282+'px'},300);
            }else{
                $('.ag_af_nav').hide();
                $('.line_hover').animate({'left':-35+'px'},300);
            }
    });
...
var fenyeCro ;
var xfenlLen ,xfenNum,xfenTxt;
var liHtml ='';
var n = 1;
function  download_xfl(){
    $(' .do_dfl').eq(defaultNum).css('display','block');
    $('.do_xtab .tablable').click(function(){
        $(' .do_dfl').eq(defaultNum).hide().empty();
        
        $('.do_xtab .tablable').css({'color':'#fff','background':'url("http://s-event.cn/EDM/dell/161011reg/images/jt.png") no-repeat left center'}).hover(
                function(){
                    $(this).css({'color':'#000','background':'url("http://s-event.cn/EDM/dell/161011reg/images/jt1.png") no-repeat left center'});
                    
                },function(){

                    $(this).css({'color':'#fff','background':'url("http://s-event.cn/EDM/dell/161011reg/images/jt.png") no-repeat left center'});
                }
        );
        $(this).css({'color':'#000','background':'url("http://s-event.cn/EDM/dell/161011reg/images/jt1.png") no-repeat left center'}).hover(
            function(){
                $(this).css({'color':'#000','background':'url("http://s-event.cn/EDM/dell/161011reg/images/jt1.png") no-repeat left center'});
            },function(){
                $(this).css({'color':'#000','background':'url("http://s-event.cn/EDM/dell/161011reg/images/jt1.png") no-repeat left center'});
        ...
        xfenlLen = ziliao[defaultNum][xfenTxt].length;
        console.log('小分类里的资料数量：'+xfenlLen);
        
        
        
        
        // for(var j=0;j<xfenlLen;j++){
            
        //     if( j%3==0){
        //         liHtml+='<li class="doli01">'+
        //             '<a  title="'+ziliao[defaultNum][xfenTxt][j]+'下载" href="http://k-ad.cn/bg/view/dellmeet/pdf/'+ziliao[defaultNum][xfenTxt][j]+'" >'+
        //             '<p class="doimg">'+
        //                 '<img src="./images/'+ziliaoImg[defaultNum].zlImg[xfenNum]+'">'+
        //             '</p>'+
        //             '<p class="dotxt">'+ziliao[defaultNum][xfenTxt][j]+'</p>'+
        //             '<p class="dobtn"></p>'+
        //             '</a>'+
        //         '</li>';
        //     }else{
        ...
        if(xfenlLen > 6){
            fenyeCro = Math.ceil(xfenlLen/6);
            console.log('分页器分为几个——'+fenyeCro);
            $('.do_list').css('display','block'); 
            if(n<=1){
              fenyeCla(n);  
            }
        ...
        $('.do_list').find('span').eq(2).click(function(){
            console.log($(' .do_dfl').eq(defaultNum));
            $(' .do_dfl').eq(defaultNum).empty();
            console.log('下一页点击');

            n++;
            fenyeCla(n);

            // $(' .do_dfl').eq(defaultNum).append(liHtml).fadeIn();
...
function  fenyeCla(num){
    $(' .do_dfl').eq(defaultNum).hide().empty();
    for(var j=num*6-5;j<=num*6;j++){
                console.log('hahahha'+ziliao[defaultNum][xfenTxt][j-1]);
            
                if( j%3==0){
                    liHtml+='<li class="doli01">'+
                        '<a  title="'+ziliao[defaultNum][xfenTxt][j-1]+'下载" href="http://k-ad.cn/bg/view/dellmeet/pdf/'+ziliao[defaultNum][xfenTxt][j]+'" >'+
                        '<p class="doimg">'+
                            '<img src="./images/'+ziliaoImg[defaultNum].zlImg[xfenNum]+'">'+
                        '</p>'+
                        '<p class="dotxt">'+ziliao[defaultNum][xfenTxt][j-1]+'</p>'+
                        '<p class="dobtn"></p>'+
                        '</a>'+
                    '</li>';
                }else{
...
var ziliaoImg =[{
    zlImg:['do_2-1.jpg','do_2-2.jpg','do_2-3.jpg','do_2-4.jpg'],
},{
    zlImg:['do_3-1.jpg','do_3-2.jpg','do_3-3.jpg','do_3-4.jpg','do_3-5.jpg'],
},{
    zlImg:['do_4-1.jpg','do_4-2.jpg','do_4-3.jpg','do_4-4.jpg'],
}];            
var ziliao =[{
        'DT-OptiPlex':['OptiPlex 22 3000 Series AIO_3240 Technical.pdf',
                     'Optiplex 3046.pdf',
                     'OptiPlex 7040 Technical.pdf',
                     'Optiplex 7040 unite.pdf'
                    ],
        'NB-Latitude':['Latitude 11 5000 Series 2-in-1 (5175, 5179).pdf',
        ...
        'WS-Precision':['Dell Precision 15 5000 Series-5510.pdf',
        ...
        'FY17Q4 CSG':['FY17Q4 CSG 产品手册.pdf'],
    },{
        'Catalog':['dell anycloud solution3m.pdf',
                   'VSS Brochure.pdf',
        ],
        'PowerEdge':['FY17Q2 poweredge catalog.pdf',
                     'PE C4130_intel.pdf',
                     'PE C6320_intel.pdf',
                     'PE FC430_intel.pdf',
                     'PE FC630_intel.pdf',
        ...
        'Networking':['Dell Networking H9106 H9124.pdf',
        ...
        'Storage':['Compellent flash storage optimized solutions.pdf',
        ...
        'Telecom':['BCF_VMWare Private Cloud_CN.pdf',
